...
module.exports = (function() {
     ...
     */
    var Workbook = function(data, callback, options) {
        var self = this;
        ...
        if(data) {
            self.loadTemplate(data, callback);
        }
    };
...
//
    ...
    */
     ...
     */
    Workbook.prototype.loadTemplate = function(data, callback) {
        var self = this;
        ...
        self.archive = new JSZip();

            //self.archive = new zip(data, {base64: false, checkCRC32: true});
            // more files !
            self.archive.loadAsync(data).then(function(zip) {
                // you now have every files contained in the loaded zip
                self.archive = zip



                self.archive.file("_rels/.rels").async("string").then(function (text) {
                    // Load relationships
                    var rels = etree.parse(text).getroot();
                    var workbookPath = rels.find("Relationship[@Type='" + DOCUMENT_RELATIONSHIP + "']").attrib.Target;
                    self.workbookPath = workbookPath;
                    self.prefix       = path.dirname(workbookPath);



                    self.archive.file(workbookPath).async("string").then(function (workbook) {
                        self.workbook = etree.parse(workbook).getroot();
                    
                        var relsPath = self.prefix + "/" + '_rels' + "/" + path.basename(workbookPath) + '.rels'
                        self.archive.file(relsPath).async("string").then(function (workbookRels) {
                            self.workbookRels = etree.parse(workbookRels).getroot();
                            ...
                            self.sharedStrings = [];
                            self.archive.file(self.sharedStringsPath).async("string").then(function (sharedStrings) {

                                etree.parse(sharedStrings).getroot().findall('si').forEach(function(si) {
                                    var t = {text:''};
     ...
     */
    Workbook.prototype.substitute = function(sheetName, substitutions, callback) {
        var self = this;


        self.loadSheet(sheetName).then((sheet)=>{
            ...
            //temporary file to the rows
            let pahtTempRows = new tmp.File();

            let saveRowsOnDisk = (rows)=>{
                //we use a temp file for write the rows temporary 
                let sb = []

                for (var i = 0; i < rows.length; i++) {
                    sb.push(etree.tostring(rows[i], {xml_declaration: false}))
                }

                fs.appendFileSync(pahtTempRows.path, sb.join(""))  
                sb = null
                return []
            }
                                        

            self.loadTables(sheet.root, sheet.filename).then((namedTables)=>{
                    ...
                    row.findall("c").forEach(function(cell) {
                        ...
                        if(cell.attrib.t === "s") {
                            ...
                            self.extractPlaceholders(string).forEach(function(placeholder) {
                                ...
                                var substitution = substitutions[placeholder.name] || null
                                ...
                                if (substitution) {
                                    ...
                                    if(placeholder.full && placeholder.type === "table" && substitution instanceof Array) {
                                        ...
                                        rows.push(row);
                                    } else if(placeholder.full && placeholder.type === "matriz" && substitution instanceof Array) {

                                        //1) insert each item as array
                                        let len = substitution.length
                                        
                                        appendCell = false; // don't double-insert cells

                                        //2 we divide the data in chunks
                                        let index = 0;
                                        let sizeBlocks = 1000;
                                        let result = [];
                                        let slice = (source, index) => source.slice(index, index + sizeBlocks);

                                        while (index < len) {
                                          let temp = slice(substitution, index);
                                          result.push(temp);
                                          index += sizeBlocks;
                                        }

                                        rows = saveRowsOnDisk(rows)

                                        let processChunk = (indexTemp, chunk)=>{


                                             for (var i = indexTemp; i < chunk.length; i++) {
                                                    let dataRow = chunk[i]
                                                    var newRow
                                                    var newCell
                                                    var newCellsInsertedOnNewRow = 0
                                                    var newCells = []
                                                    ++totalRowsInserted

                                                    // create one row to use.
                                                    ...
                                                    //insert values in the new row
                                                    newCellsInsertedOnNewRow = self.substituteArray(newCells, newCell, dataRow);
                                                ...
                                                rows = saveRowsOnDisk(newTableRows)

                                                newTableRows = []
                                                self.pushDown(self.workbook, sheet.root, namedTables, currentRow, size);
                                                currentRow += size
                                                size = 0
                                        }
                                        ...
                                        for (var k = 0; k < result.length; k++) {
                                            ...
                                            console.log("process chunk " + (k +1)  + " de " +  result.length)

                                            if(k === 0) {
                                                //first line of all arrays only first chunk
                                                let dataRow = chunk[k]
                                                // insert in the row where the placeholders are
                                                let newCellsInserted = self.substituteArray(
                                                    cells, cell, dataRow
                                                );
                                                ...
                                                rows.push(row);
                                                rows = saveRowsOnDisk(rows)
                                                // ommit the first register
                                                processChunk(1, chunk)
                                            } else { 
                                               processChunk(0, chunk)
                                             }
                                    ...
                                    console.log('No hay valores para reemplazar : ' + placeholder.name)
                                }
                            });
                        }
                ...
                self.clearChildrens(sheetData);
                    ...
                    let pahtTemp = new tmp.File();
                    let templateSheet = etree.tostring(sheet.root).split('<sheetData />')
                    
                    let initTemplate = Buffer.from(templateSheet[0], 'utf8');
                    let endTemplate = Buffer.from(templateSheet[1], 'utf8');
                    templateSheet = null

                    let initTag = Buffer.from('<sheetData>', 'utf8');
                    let endTag = Buffer.from('</sheetData>', 'utf8');
                    let dataRows  = fs.readFileSync(pahtTempRows.path)

                    var arr = [initTemplate, initTag,dataRows ,endTag, endTemplate];
                    var buf = Buffer.concat(arr);

                    self.archive.file(sheet.filename, buf,{
                        compression: "DEFLATE",
                        compressionOptions: {
     ...
     */
    Workbook.prototype.generate = function(options, callback) {
        var self = this;
       ...
       self.archive.generateNodeStream({type:'nodebuffer', streamFiles:true, base64: false})
        .pipe(fs.createWriteStream( __dirname + options.namezip + '.xlsx'))
        .on('finish', function () {
            // JSZip generates a readable stream with a "end" event,
            // but is piped here in a writable stream which emits a "finish" event.
            console.log("out.xlsx written." + options.namezip + '.xlsx');
            callback( __dirname + options.namezip + '.xlsx');
        });

      /*  self.archive.generateAsync(options).then(function(content) {
            callback(content);
        });*/
    };
    ...
    Workbook.prototype.writeSharedStrings = function() {
        var self = this;
         ...
         return  new Promise((resolve, reject)=>{
            ...
            let size = self.sharedStrings.length
            var sb = [
                '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>',
                '<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="',size,'" uniqueCount="',size,'">'
            ]

            let pathTempStrings = new tmp.File();

            /*
             * XML escaped characters
             * XML special characters get escaped at install time as follows:
             * https://www.advancedinstaller.com/user-guide/xml-escaped-chars.html
             */
            let replaceCharacters = (string)=>{
                string = string.replace(/[&]/mgi, '&amp;')
                string = string.replace(/[<]/mgi, '&lt;')
                string = string.replace(/[>]/mgi, '&gt;')
                string = string.replace(/["]/mgi, '&quot;')
                string = string.replace(/[']/mgi, '&apos;')
            ...
            self.sharedStrings.forEach(function(string) {
                sb.push(`<si><t>${replaceCharacters(string)}</t></si>`)
                if (sb.length > 5000) {
                  fs.appendFileSync(pathTempStrings.path, sb.join(""))  
                  sb = []
                } 
            });
            ...
            self.archive.file(self.sharedStringsPath, fs.readFileSync(pathTempStrings.path), {
                compression: "DEFLATE",
                compressionOptions: {
                    level: 9 // force a compression and a compression level for this file
                }
    ...
    Workbook.prototype.loadSheet = function(sheet) {
        ...
        return  new Promise((resolve, reject)=>{
            self.archive.file(info.filename).async("string").then(function (content) {
                // use content
                resolve({
    ...
    Workbook.prototype.loadTables = function(sheet, sheetFilename) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            self.archive.file(relsFilename).async("string").then(function (relsFile) {
                 if(relsFile === null) {
                   resolve(tables);    
                }

                var rels = etree.parse(relsFile).getroot();
                ...
                sheet.findall("tableParts/tablePart").forEach(function(tablePart) {
                    ...
                    promises.push(
                        self.archive.file(tableFilename).async("string").then(function (contentTable) {
                            // use content
                            tableTree = etree.parse(contentTable);
    ...
    Workbook.prototype.substituteHyperlinks = function(sheetFilename, substitutions) {
        ...
        return  new Promise((resolve, reject)=>{
            ...
            relsFile.async("string").then(function (relsFileText) {
              const rels = etree.parse(relsFileText).getroot();
    ...
    Workbook.prototype.replaceChildren = function(parent, children) {
     ...
     // Empty all children of `parent` with the nodes in the list `children`
    Workbook.prototype.clearChildrens = function(parent) {
        parent.delSlice(0, parent.len());
    };
